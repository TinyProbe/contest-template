...
class FastIO {
  ...
  bool state;
private:
  inline i8 sys_getchar() { return pb==pe && (pe=(pb=i_buffer)+fread(i_buffer, sizeof(u8), MAX_SIZE, stdin), pb==pe) ? (state=false) : *pb++; }
...
public:
  ...
  ~FastIO() { assert(o_buffer.size()==fwrite(o_buffer.c_str(), sizeof(i8), o_buffer.size(), stdout)); }
  template <typename T>
